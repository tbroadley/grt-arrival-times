...
function processData({ stops, trips, calendarDates, stopTimes, stopTimeUpdates }, timeHorizon) {
  ...
  request.get('https://www.regionofwaterloo.ca/opendatadownloads/GRT_GTFS.zip')
    ...
    .on('close', () => {
      request.get('http://192.237.29.212:8080/gtfsrealtime/TripUpdates')
        ...
        .on('finish', () => cb(processData({ stops, trips, calendarDates, stopTimes, stopTimeUpdates }, timeHorizon)));
    });
}
